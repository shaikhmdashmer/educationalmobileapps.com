...
export default function UserStudentsswithPage({ pagedata, allcategoriesdata, rightsidedata }) {
    ...
    const withoutSearch = pagedata && pagedata.map(carddata => {
        return (
                            ...
                            <div className="categoriesSpace user-categoriesSpace">
                                ...
                                <div className={router.asPath == `/user/students-with-special-needs/` ? 'new-radiobtnlink new-radiobtnlink-active' : 'new-radiobtnlink'}>
                                    <label htmlFor='userLink3'>
                                        <input type="radio" name="categorynamelink" id='userLink3' onChange={(e) => { setcategorylinkfield(e.target.checked); setcategoryvalueTrue(`students-with-special-needs`); router.push(`students-with-special-needs`)}} required />
                                ...
                                <div className="headingSec">
                                    <div>
                                        <p className="musicPara">
                                            Showing results for {pagedata?.length} apps
                                        </p>
                                    </div>
                                </div>
                            </div>
...
export async function getStaticProps() {
    const pagedata = catdatalist.filter(p => p.user.toString() == 'Students with Special Needs');
    const allcategoriesdata = catdatalist

    // right sidebar fetching data
    const rightsideres = await fetch(`https://admin.educationalmobileapps.com/educationappcategories?user=Students with Special Needs`);
